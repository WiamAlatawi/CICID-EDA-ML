...
def load_data():
    import kagglehub
    os.environ["KAGGLE_API_TOKEN"] = st.secrets["kaggle"]["key"]
    path = kagglehub.dataset_download("elshewey/intrusion-detection-cicids2017")
    # نقرأ sample مباشرة بدون ما نحمّل كل الداتا
    df_full = pd.read_csv(f'{path}/cicids2017_binary_balanced.csv')
    # sample متوازن 25k normal + 25k attack = 50k صف بس
    df = df_full.groupby('Attack_Binary', group_keys=False).apply(
